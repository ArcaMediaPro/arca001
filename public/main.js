...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Este listener se asegura de que la UI se refresque si el usuario vuelve atrás en el historial.
    window.addEventListener('pageshow', function(event) {
        if (event.persisted) {
            console.log('Página cargada desde bfcache. Forzando actualización de UI.');
            
            // La única fuente de la verdad es la cookie.
            const lang = getCookie('preferredLanguage') || 'es';
            
            // Re-aplicamos todas las traducciones y elementos visuales.
            applyPageTranslations();
            updateLocalizedImages(lang);
            updatePlanCounterUI();
            
            // Nos aseguramos de que el selector de idioma refleje el estado correcto.
